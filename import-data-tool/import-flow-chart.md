...
flowchart TD
    %% Main Flow
    Start([Start]) --> PrepDoc[Prepare Document]
    PrepDoc --> Upload[Upload to Frontend]
    Upload --> BE{Backend Processing}
    ...
    subgraph AIFlow[AI Processing Flow]
        direction TB
        PrepareData[Prepare Data] --> ValidatePrep{Validate<br/>Output}
    ...
    class PrepDoc,Upload,SaveImport,PrepareData,MapStructure,ValidateData,UpdateStatus,Preview,CreateQuiz,SaveDB process
    class BE,ValidatePrep,ValidateMap,ValidateFinal,FEPoll,TeacherReview,FinalValidate decision
```